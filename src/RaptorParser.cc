...
int RaptorParser::parseChunk(std::string buffer, bool isEnd)
{
  const unsigned char* buffer_c = NULL;
  size_t buffer_len_c = 0;
  if (buffer.length() > 0) {
  ...
  return raptor_parse_chunk(parser_, buffer_c, buffer_len_c, (int)isEnd);
}
...
int RaptorParser::parseUri(RaptorUri* uri, RaptorUri* base_uri)
{
  ...
  std::cout << "Triples found: " << triples_->size()  << std::endl;
